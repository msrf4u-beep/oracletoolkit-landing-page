...
function otRenderDeliverables(){
  ...
  if(!items.length){
    el.innerHTML = '<div class="empty">No saved deliverables yet. Save a RICE Tracker from the SOW accelerator.</div>';
    return;
  }
  el.innerHTML = items.map(d => {
    const created = d.created_at ? new Date(d.created_at).toLocaleString() : "";
    return `<article class="run-card deliverable-card">
      <div class="run-card-top"><strong>📘 ${otEscape(d.title||"Saved Deliverable")}</strong><em>${otEscape(d.status||"Generated")}</em></div>
      <p><strong>Type:</strong> ${otEscape(d.deliverable_type||"")}<br>
      <strong>Source:</strong> ${otEscape(d.source_accelerator||"")}<br>
      <strong>Items:</strong> ${otEscape(d.item_count||0)}<br>
