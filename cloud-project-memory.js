...
async function otDeleteMemory(t,id,silent=false){if(!silent && !confirm("Delete this saved OracleToolkit item? This cannot be undone."))return;const c=await otGetSupabaseClient();if(!c)return;const{error}=await c.from(t).delete().eq("id",id);if(error){otStatus(`Delete failed: ${error.message}`,"error");return}await otLoadAllMemory();otRender();otStatus("Project memory item deleted.","success")}
...
function otCoaMemoryLaunchUrl(){
  const p = otCurrentProject();
  if(!p || !p.id || !p.project_memory_key) return "";
  const base = "https://coaacceleratormvp-immxr6kprgxex4wkhomcby.streamlit.app/";
  const params = new URLSearchParams({
    project_id: p.id,
    project_memory_key: p.project_memory_key,
    clerk_user_id: p.clerk_user_id || "",
...
async function otBulkDeleteDeliverables(){
  const ids = Array.from(document.querySelectorAll(".deliverable-check:checked")).map(x=>x.value).filter(Boolean);
  if(!ids.length){ otStatus("No deliverables selected.", "error"); return; }
  if(!confirm(`Delete ${ids.length} selected deliverable(s)? This cannot be undone.`)) return;
  for(const id of ids){ await otDeleteMemory(OT_TABLES.deliverables, id, true); }
  await otLoadAll();
  otStatus(`Deleted ${ids.length} deliverable(s).`, "success");
}
async function otBulkDeleteMemoryRows(){
  const selected = Array.from(document.querySelectorAll(".memory-row-check:checked")).map(x=>({id:x.value, table:x.getAttribute("data-table")})).filter(x=>x.id&&x.table);
  if(!selected.length){ otStatus("No detailed rows selected.", "error"); return; }
  if(!confirm(`Delete ${selected.length} selected detailed memory row(s)? This cannot be undone.`)) return;
  for(const row of selected){ await otDeleteMemory(row.table, row.id, true); }
  await otLoadAll();
  otStatus(`Deleted ${selected.length} detailed row(s).`, "success");
}
function otWireBulkDelete(){
  const bd = document.getElementById("bulk-delete-deliverables");
  if(bd) bd.onclick = otBulkDeleteDeliverables;
  const bm = document.getElementById("bulk-delete-memory-rows");
  if(bm) bm.onclick = otBulkDeleteMemoryRows;
}


function otDiscoveryMemoryLaunchUrl(){
  const p = otCurrentProject();
  if(!p || !p.id || !p.project_memory_key) return "";
  const base = "https://discoverycommandcenterpro-fu9jxdqbb4qtc28jexi7ul.streamlit.app/";
  const params = new URLSearchParams({
    project_id: p.id,
    project_memory_key: p.project_memory_key,
    clerk_user_id: p.clerk_user_id || "",
...
function otCleanupDeliverableLaunchArtifacts(){
  try{
    ...
    scope.querySelectorAll("a").forEach(a=>{
      ...
      if(txt === "Launch" && !a.classList.contains("download-json") && !a.classList.contains("download-csv")){
        const href = a.getAttribute("href") || "";
        if(href.includes("streamlit.app") || href === "#") a.remove();
      }
    });
  }catch(e){ console.warn("Deliverable launch cleanup skipped", e); }
}
...
// v231_alpha3_deliverable_cleanup_timer
setInterval(()=>{try{otCleanupDeliverableLaunchArtifacts();}catch(e){}},1500);
